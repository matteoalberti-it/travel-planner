...
export default function BucketSection({ tripId }: { tripId: string }) {
  ...
  const handleAddCategory = async () => {
    ...
    if (!user) return
    await supabase.from('bucket_categories').insert({ trip_id: tripId, name: newCategoryName, emoji: newCategoryEmoji, created_by: user.id })
    setNewCategoryName('')
    setNewCategoryEmoji('📌')
  ...
  const handleUpdateCategory = async () => {
    ...
    if (activeCategory?.id === editingCategory.id) setActiveCategory({ ...activeCategory, name: editingCategory.name, emoji: editingCategory.emoji })
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    ...
    await supabase.from('bucket_items').delete().eq('category_id', categoryId)
    await supabase.from('bucket_categories').delete().eq('id', categoryId)
        ...
        {editingCategory && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => handleDeleteCategory(editingCategory.id)} className="px-4 border border-red-500/30 text-red-400 py-2.5 rounded-xl text-sm hover:border-red-500/60">🗑️ Elimina</button>
              ...
              {editingItem?.id === item.id ? (
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  ...
                  <div className="flex gap-3 mt-4">
                    <button onClick={() => handleDeleteItem(editingItem.id)} className="px-4 border border-red-500/30 text-red-400 py-2.5 rounded-xl text-sm hover:border-red-500/60">🗑️</button>
                ...
                <div className="bg-white/3 border border-white/8 rounded-xl px-5 py-3 flex items-center gap-4 hover:border-white/20 transition-colors group">
                  ...
                  <button onClick={() => setEditingItem(item)} className="text-white/20 hover:text-white opacity-0 group-hover:opacity-100 transition-all shrink-0 text-sm">✏️</button>
                </div>
              )}
            </div>
          ))}
